...
def velocity_calculation(Velocity, sensor, sampletime):
	#Calculate velocity V = V0 + at and distance traveled dx = Vo*t +1/2 * a *t^2

	accel = math.sqrt(sensor.linear_acceleration[0]**2 + sensor.linear_acceleration[1]**2)
	Velocity = Velocity + accel * sampletime
	dx = Velocity * sampletime + 1/2 * accel * (sampletime**2) 
	
	

	print("Velocity: {} m/s, dx: {}  ,accel: {},  sampletime: {}" .format(Velocity, dx, accel, sampletime))
	return dx
